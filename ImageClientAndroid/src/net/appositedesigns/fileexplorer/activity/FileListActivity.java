...
import net.appositedesigns.fileexplorer.workers.FileDownloader;
import net.appositedesigns.fileexplorer.workers.FileMover;
import net.appositedesigns.fileexplorer.workers.FileUploader;
...
public class FileListActivity extends BaseFileListActivity {
    ...
    public String getBackupUrl() {
        return prefs.getBackupUrl();
    }

    public String getBackupDir() {
        return prefs.getBackupDir();
    }
		...
		explorerListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (explorerListView.isClickable()) {
                    ...
                    if(file.getOnline()) {
                        downloader= new FileDownloader(FileListActivity.this);
                        downloader.execute(prefs.getBackupUrl(),file.getName(),"");
                    } else {
                        select(file.getPath());
                    }
				}
	...
	void select(File file) {
		if (Util.isProtected(file)){
			...
			doFileAction(file);
            //new FileUploader(this).execute(prefs.getBackupUrl(),file.getPath());
		}
	}
    ...
    public void listContentsOnline() {
        new Finder(this).execute(new File("/Go_Online"));
    }
